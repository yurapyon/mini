...
const Register_ = struct {
    offset: vm.Cell,

    pub fn fetchThenIncrement(
        self: *Register,
        memory: []u8,
        range: Range,
        count: usize,
    ...
    pub fn fetchThenDecrement(
        ...
        self.storeSubtract(memory, count);
        return addr;
    }

    pub fn fetchThenAdvanceByte(
        self: *Register,
        memory: []u8,
        range: Range,
    ) ?vm.Cell {
...
// Stack.push(mini.memory, mini.data_stack_start, mini.data_stack, value,);
// mini.push(value);
...
const Stack_ = struct {
    pub fn push_(
        ...
        const addr = top.fetch(memory);
        if (addr + 1 > range.end) {
            return null;
        }
        self.storeAdd(memory, 2);
        memory[addr] = value;
    }
    ...
    pub fn pop_(
        ...
        const top_addr = top.fetch(memory);
        if (top_addr <= range.start) {
            return error.StackUnderflow;
        }
        top.storeSubtract(memory, 1);
        return memory[top.fetch(memory)];
    }
